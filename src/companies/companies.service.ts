...
export class CompaniesService {
    ...
    private generateCompanyId (): string {
        ...
        return `COMP-${ new Date().getFullYear() }-${ timestamp }${ random }`;
    }

    private async generateUniqueCompanyId (): Promise<string> {
        ...
        if ( !isUnique )
        {
            // Fallback: use timestamp with more randomness
            companyId = `COMP-${ new Date().getTime() }-${ Math.random().toString( 36 ).substring( 2, 8 ).toUpperCase() }`;
        }
    ...
    async create ( createCompanyDto: CreateCompanyDto ): Promise<any> {
        try
        {
            ...
            const companyId = await this.generateUniqueCompanyId();

            const newCompany = this.companyRepository.create( {
                ...createCompanyDto,
                companyId, // Add the generated companyId
            } );
